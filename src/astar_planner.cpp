...
namespace pairs_octomap_planner
{
  ...
  AstarPlanner::AstarPlanner(const std::shared_ptr<rclcpp::Node>& node, std::string node_name, double safe_obstacle_distance, double euclidean_distance_cutoff,
                             double submap_distance, double planning_tree_resolution, double distance_penalty, double greedy_penalty, double timeout_threshold,
                             double max_waypoint_distance, double min_altitude, double max_altitude, bool unknown_is_occupied,
                             std::shared_ptr<pairs_lib::BatchVisualizer> bv)
  {
                                                                        ...
                                                                        std::shared_ptr<octomap::OcTree> mapping_tree, const double timeout)
  {
    ...
    RCLCPP_INFO_THROTTLE(m_node->get_logger(), *m_node->get_clock(), 1000, "[Astar]: the planning tree took %.4f s to create",
                         (m_node->now() - time_start_planning_tree).seconds());
    ...
    if (map_query == NULL)
    {
      ...
      original_goal = false;
    } else if (map_query->getValue() == float(TreeValue::OCCUPIED))
    {
    ...
    pairs_lib::geometry::Cuboid c_start(Eigen::Vector3d(start_coord.x(), start_coord.y(), start_coord.z()), Eigen::Vector3d(0.3, 0.3, 0.3),
                                      Eigen::Quaterniond::Identity());
    pairs_lib::geometry::Cuboid c_goal(Eigen::Vector3d(map_goal.x(), map_goal.y(), map_goal.z()), Eigen::Vector3d(0.3, 0.3, 0.3), Eigen::Quaterniond::Identity());
    ...
    while (!open.empty() && rclcpp::ok())
    {
      ...
      if ((time_now - time_start).seconds() > timeout_threshold)
      {
        ...
        RCLCPP_WARN(m_node->get_logger(), "[Astar]: Planning timeout (%.4f s after start of search)! Using current best node as goal.",
                    (m_node->now() - time_start_planning).seconds());
      ...
      if (distEuclidean(current_coord, map_goal) <= 2 * planning_tree_resolution)
      {
        ...
        RCLCPP_INFO(m_node->get_logger(), "[Astar]: Path found. Length: %ld. Search time: %.4f", path_keys.size(),
                    (m_node->now() - time_start_planning).seconds());
      ...
      /* RCLCPP_INFO(m_node->get_logger(),"[%s]: iter %d, open %d, closed %d, neighbours %d", ros::this_node::getName().c_str(), iter++, open.size(),
       * closed.size(), neighbors.size()); */
    ...
    if (best_node != first)
    {
      ...
      RCLCPP_INFO(m_node->get_logger(), "[Astar]: direct path does not exist, going to the 'best_node', search time: %.4f",
                  (m_node->now() - time_start_planning).seconds());
    ...
    if (best_node_greedy != first)
    {
      ...
      RCLCPP_INFO(m_node->get_logger(), "[Astar]: direct path does not exist, going to the best_node_greedy', search time: %.4f",
                  (m_node->now() - time_start_planning).seconds());
  ...
  std::vector<octomap::OcTreeKey> AstarPlanner::getNeighborhood(const octomap::OcTreeKey& key, octomap::OcTree& tree)
  {
    ...
    for (auto& d : EXPANSION_DIRECTIONS)
    {
      ...
      {
        // free cell?
        if (tree_node->getValue() == float(TreeValue::FREE) && tree.keyToCoord(newkey).z() >= min_altitude && tree.keyToCoord(newkey).z() <= max_altitude)
        {
  ...
  octomap::OcTreeKey AstarPlanner::expand(const octomap::OcTreeKey& key, const std::vector<int>& direction, [[maybe_unused]] octomap::OcTree& tree)
  {
  ...
  bool AstarPlanner::freeStraightPath(const octomap::point3d p1, const octomap::point3d p2, octomap::OcTree& tree)
  {
    ...
    for (auto& k : ray)
    {
      ...
      if (tree_node->getValue() == float(TreeValue::OCCUPIED))
      {
  ...
  DynamicEDTOctomap AstarPlanner::euclideanDistanceTransform(std::shared_ptr<octomap::OcTree> tree, [[maybe_unused]] const octomap::point3d& orig_coord)
  {
  ...
  std::optional<std::pair<std::shared_ptr<octomap::OcTree>, std::vector<octomap::point3d>>>
  AstarPlanner::createPlanningTree(std::shared_ptr<octomap::OcTree> tree, const octomap::point3d& start, double resolution, const octomap::point3d& orig_coord,
                                   [[maybe_unused]] double radius)
  {
    ...
    if (binary_tree_query != NULL && binary_tree_query->getValue() != float(TreeValue::FREE))
    {
  ...
  octomap::point3d AstarPlanner::nearestFreeCoord(const octomap::point3d& p, const octomap::point3d& uav_pos, octomap::OcTree& tree)
  {
    ...
    if (query != NULL && query->getValue() == float(TreeValue::FREE))
    {
      return p;
    }
    ...
    return nearestFreeCoord(new_p, uav_pos, tree);
  }
  //}
  ...
  std::vector<octomap::point3d> AstarPlanner::postprocessPath(const std::vector<octomap::point3d>& waypoints, octomap::OcTree& tree)
  {
    ...
    for (unsigned int i = 1; i < waypoints_size; i++)
    {
  ...
  std::vector<octomap::point3d> AstarPlanner::prepareOutputPath(const std::vector<octomap::OcTreeKey>& keys, octomap::OcTree& tree)
  {
    ...
    for (size_t i = 1; i < processed.size(); i++)
    {
      ...
      bv->addRay(pairs_lib::geometry::Ray::twopointCast(w0, w1), 0.4, 0.4, 1.0, 1);
    }
  ...
  void AstarPlanner::visualizeTreeCubes(octomap::OcTree& tree, bool show_unoccupied)
  {
    ...
    for (auto it = tree.begin(); it != tree.end(); it++)
    {
      ...
      pairs_lib::geometry::Cuboid c(center, size, orientation);

      if (it->getValue() == float(TreeValue::OCCUPIED))
      {
      ...
      if (show_unoccupied && it->getValue() == float(TreeValue::FREE))
      {
  ...
  octomap::point3d AstarPlanner::generateTemporaryGoal(const octomap::point3d& start, const octomap::point3d& goal, octomap::OcTree& tree)
  {
    ...
    for (auto& k : ray)
    {
      ...
      if (tree.search(coords) != NULL && tree.search(coords)->getValue() == float(TreeValue::FREE))
      {
        temp_goal = coords;
      }
    }
