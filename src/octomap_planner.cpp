...
#include <pairs_msgs/MpcPredictionFullState.h>
...
#include <pairs_modules_msgs/OctomapPlannerDiagnostics.h>
...
private:
  ...
  pairs_lib::SubscribeHandler<pairs_msgs::DynamicsConstraints>       sh_constraints_;
  ...
  bool callbackGoto(pairs_msgs::Vec4::Request& req, pairs_msgs::Vec4::Response& res);
  bool callbackStop(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res);
  bool callbackReference(pairs_msgs::ReferenceStampedSrv::Request& req, pairs_msgs::ReferenceStampedSrv::Response& res);
  bool callbackSetPlanner(pairs_msgs::String::Request& req, pairs_msgs::String::Response& res);
  bool callbackSetSafetyDistance(pairs_msgs::Vec1::Request& req, pairs_msgs::Vec1::Response& res);
  bool callbackSetMaxAltitude(pairs_msgs::Vec1::Request& req, pairs_msgs::Vec1::Response& res);
  bool callbackAddVirtualObstacle(pairs_msgs::ValidateReferenceArray::Request& req, pairs_msgs::ValidateReferenceArray::Response& res);
  bool callbackRemoveVirtualObstacles(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res);

  // service clients
  pairs_lib::ServiceClientHandler<pairs_msgs::GetPathSrv>             sc_get_trajectory_;
  pairs_lib::ServiceClientHandler<pairs_msgs::TrajectoryReferenceSrv> sc_trajectory_reference_;
  pairs_lib::ServiceClientHandler<std_srvs::Trigger>                sc_hover_;
  ...
  // diagnostics
  pairs_modules_msgs::OctomapPlannerDiagnostics diagnostics_;
  ...
  pairs_msgs::Reference user_goal_;
  ...
  // routines
  void setReplanningPoint(const pairs_msgs::TrajectoryReference& traj);
   ...
   */
  std::optional<pairs_msgs::ReferenceStamped> getInitialCondition(const ros::Time time);
...
void OctomapPlanner::onInit() {
  ...
  ROS_INFO("[PairsOctomapPlanner]: initializing");

  pairs_lib::ParamLoader param_loader(nh_, "PairsOctomapPlanner");
  ...
  if (!param_loader.loadedSuccessfully()) {
    ROS_ERROR("[PairsOctomapPlanner]: could not load all parameters");
    ros::shutdown();
  }
  ...
  if (_restart_planner_on_deadlock_) {
    ...
    if (_planner_deadlock_timeout_factor < 3.0) {
      ROS_WARN(
          "[PairsOctomapPlanner]: Timeout factor for planner deadlock detection was set too low (< 3.0). Setting factor to 3.0 to prevent premature killing of "
          "the planner.");
      _planner_deadlock_timeout_factor = 3.0;
    }

    planner_deadlock_timeout_ = _planner_deadlock_timeout_factor * _timeout_threshold_;
    ROS_INFO("[PairsOctomapPlanner]: Planner deadlock timeout set to %.2f s.", planner_deadlock_timeout_);
  }
  ...
  pub_diagnostics_       = nh_.advertise<pairs_modules_msgs::OctomapPlannerDiagnostics>("diagnostics_out", 1);
  ...
  sh_tracker_cmd_ = pairs_lib::SubscribeHandler<pairs_msgs::TrackerCommand>(shopts, "tracker_cmd_in", ros::Duration(3.0), &OctomapPlanner::timeoutTrackerCmd, this);
  sh_octomap_     = pairs_lib::SubscribeHandler<octomap_msgs::Octomap>(shopts, "octomap_in", ros::Duration(5.0), &OctomapPlanner::timeoutOctomap, this,
                                                                 &OctomapPlanner::callbackOctomap, this);

  sh_control_manager_diag_ = pairs_lib::SubscribeHandler<pairs_msgs::ControlManagerDiagnostics>(shopts, "control_manager_diag_in", ros::Duration(3.0),
                                                                                            &OctomapPlanner::timeoutControlManagerDiag, this);

  sh_constraints_ = pairs_lib::SubscribeHandler<pairs_msgs::DynamicsConstraints>(shopts, "constraints_in");

  // | --------------------- service clients -------------------- |

  sc_get_trajectory_       = pairs_lib::ServiceClientHandler<pairs_msgs::GetPathSrv>(nh_, "trajectory_generation_out");
  sc_trajectory_reference_ = pairs_lib::ServiceClientHandler<pairs_msgs::TrajectoryReferenceSrv>(nh_, "trajectory_reference_out");
  sc_hover_                = pairs_lib::ServiceClientHandler<std_srvs::Trigger>(nh_, "hover_out");

  // | ----------------------- transformer ---------------------- |

  transformer_ = std::make_unique<pairs_lib::Transformer>("Pathfinder");
  transformer_->setDefaultPrefix(_uav_name_);
  transformer_->retryLookupNewest(true);

  // | -------------------- batch visualiuzer ------------------- |

  bv_input_ = pairs_lib::BatchVisualizer(nh_, "visualize_input", "");
  ...
  bv_processed_ = pairs_lib::BatchVisualizer(nh_, "visualize_processed", "");
  ...
  ROS_INFO("[PairsOctomapPlanner]: initialized");
}
...
void OctomapPlanner::timeoutTrackerCmd(const std::string& topic, const ros::Time& last_msg) {
  ...
  if (state_ != STATE_IDLE) {
    ...
    ROS_WARN_THROTTLE(1.0, "[PairsOctomapPlanner]: position cmd timeouted!");
...
void OctomapPlanner::callbackOctomap(const octomap_msgs::Octomap::ConstPtr msg) {
  ...
  ROS_INFO_ONCE("[PairsOctomapPlanner]: getting octomap");

  std::optional<OcTreePtr_t> octree_local = msgToMap(msg);

  if (!octree_local) {
    ROS_WARN_THROTTLE(1.0, "[PairsOctomapPlanner]: received map is empty!");
    return;
  }
...
void OctomapPlanner::timeoutOctomap(const std::string& topic, const ros::Time& last_msg) {
  ...
  if (state_ != STATE_IDLE) {
    ...
    ROS_WARN_THROTTLE(1.0, "[PairsOctomapPlanner]: octomap timeouted!");
...
void OctomapPlanner::timeoutControlManagerDiag(const std::string& topic, const ros::Time& last_msg) {
  ...
  if (state_ != STATE_IDLE) {
    ...
    ROS_WARN_THROTTLE(1.0, "[PairsOctomapPlanner]: Control manager diag timeouted!");
...
bool OctomapPlanner::callbackStop([[maybe_unused]] std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res) {
  ...
  if (!ready_to_plan_) {
    ...
    ROS_ERROR_STREAM_THROTTLE(0.5, "[PairsOctomapPlanner]: " << ss.str());
...
bool OctomapPlanner::callbackGoto(pairs_msgs::Vec4::Request& req, pairs_msgs::Vec4::Response& res) {
  ...
  {
    pairs_msgs::TrackerCommandConstPtr tracker_cmd = sh_tracker_cmd_.getMsg();

    pairs_msgs::ReferenceStamped reference;
    ...
    if (result) {
      ...
      ROS_ERROR_STREAM("[PairsOctomapPlanner]: " << ss.str());
...
bool OctomapPlanner::callbackReference(pairs_msgs::ReferenceStampedSrv::Request& req, pairs_msgs::ReferenceStampedSrv::Response& res) {
  ...
  // | -------- transform the reference to the map frame -------- |
  {
    ...
    pairs_msgs::ReferenceStamped ref_stamped;
...
bool OctomapPlanner::callbackSetPlanner(pairs_msgs::String::Request& req, pairs_msgs::String::Response& res) {

  if (!is_initialized_) {
    return false;
  }

  ROS_INFO("[PairsOctomapPlanner]: Setting planner to %s requested.", req.value.c_str());
  res.success = true;
  ...
  ROS_INFO("[PairsOctomapPlanner]: %s", res.message.c_str());
  return res.success;
}
...
bool OctomapPlanner::callbackSetSafetyDistance(pairs_msgs::Vec1::Request& req, pairs_msgs::Vec1::Response& res) {
  ...
  if (req.goal >= _safe_obstacle_distance_min_ && req.goal <= _safe_obstacle_distance_max_) {
    ...
    ROS_INFO("[PairsOctomapPlanner]: setting safety distance to %.2f m.", _safe_obstacle_distance_);
    res.success = true;

  } else {

    ROS_WARN("[PairsOctomapPlanner]: failed to set safety distance %.2f m (outside the allowed range [%.2f, %.2f])", req.goal, _safe_obstacle_distance_min_,
...
bool OctomapPlanner::callbackSetMaxAltitude(pairs_msgs::Vec1::Request& req, pairs_msgs::Vec1::Response& res) {
  ...
  ROS_INFO("[PairsOctomapPlanner]: setting max altitude to %.2f m.", _max_altitude_);
  res.success = true;
...
bool OctomapPlanner::callbackAddVirtualObstacle(pairs_msgs::ValidateReferenceArray::Request& req, pairs_msgs::ValidateReferenceArray::Response& res) {
  ...
  std::vector<pairs_msgs::Reference> virt_obst_in_octree_frame;
  ...
  for (auto& point_ref : req.array.array) {
    ...
    if (!res_t) {
      ...
      ROS_ERROR("[PairsOctomapPlanner]: %s", res.message.c_str());
      return true;
    }

    virt_obst_in_octree_frame.push_back(res_t.value().reference);
    ROS_INFO("[OctomapPlanner]: Transformed obstacle point = [%.2f, %.2f, %.2f]", virt_obst_in_octree_frame.back().position.x,
  ...
  ROS_INFO("[PairsOctomapPlanner] adding virtual obstacle");
...
bool OctomapPlanner::callbackRemoveVirtualObstacles(std_srvs::Trigger::Request& req, std_srvs::Trigger::Response& res) {
  ...
  ROS_INFO_STREAM("[PairsOctomapPlanner]: " << ss.str());
  res.success = true;
  res.message = ss.str();
  return true;
}
...
void OctomapPlanner::timerMain([[maybe_unused]] const ros::TimerEvent& evt) {
  ...
  if (!got_octomap || !got_tracker_cmd || !got_control_manager_diag || !got_constraints) {
    ROS_INFO_THROTTLE(1.0, "[PairsOctomapPlanner]: waiting for data: octomap = %s, position cmd = %s, ControlManager diag = %s, constraints = %s",
  ...
  ROS_INFO_ONCE("[PairsOctomapPlanner]: main timer spinning");
  ...
  const pairs_msgs::ControlManagerDiagnosticsConstPtr control_manager_diag = sh_control_manager_diag_.getMsg();
  const pairs_msgs::TrackerCommandConstPtr            tracker_cmd          = sh_tracker_cmd_.getMsg();
  ...
  switch (state_) {
    ...
    case STATE_PLANNING: {
      ...
      pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("timerMain - STATE_PLANNING", ros::Duration(_scope_timer_duration_), _scope_timer_enabled_);

      if (!octree->getRoot()) {

        ROS_ERROR("[PairsOctomapPlanner]: don't have a map");
      ...
      if (replanning_counter_ >= _max_attempts_to_replan_) {
        ...
        ROS_ERROR("[PairsOctomapPlanner]: planning failed, the uav is stuck");
      ...
      ROS_INFO("[PairsOctomapPlanner]: planning timeout %.2f s", time_for_planning);

      ros::Time init_cond_time = ros::Time::now() + ros::Duration(time_for_planning + _time_for_trajectory_generator_);

      ROS_INFO("[PairsOctomapPlanner]: init cond time %.2f s", init_cond_time.toSec());
      ...
      while (!initial_condition && iter < 20) {
        ...
        ROS_WARN("[PairsOctomapPlanner]: Trying to get initial condition updated with the last sent path.");
      ...
      if (!initial_condition) {
        ...
        ROS_ERROR_THROTTLE(1.0, "[PairsOctomapPlanner]: could not obtain initial condition for planning");
        hover();
        changeState(STATE_IDLE);
      ...
      ROS_INFO("[PairsOctomapPlanner]: init cond time stamp %.2f", initial_condition.value().header.stamp.toSec());
      ...
      if ((plan_from - user_goal_octpoint).norm() <= _min_path_length_ &&
        ...
        ROS_INFO_THROTTLE(1.0, "[PairsOctomapPlanner]: we reached the target");
        changeState(STATE_IDLE);
        break;
      }
      /*//}*/
      ...
      if ((plan_from - user_goal_octpoint).norm() <=
        ...
        if (octree->computeRayKeys(plan_from, user_goal_octpoint, key_ray)) {
          ...
          ROS_ERROR_THROTTLE(0.1, "[PairsOctomapPlanner]: collision check of short direct path failed, could not raytrace!");
          is_direct_path_collision_free = false;
          break;
        }
        ...
        if (is_direct_path_collision_free) {
          ...
          ROS_WARN_THROTTLE(1.0, "[PairsOctomapPlanner]: direct short path does not exist.");
          changeState(STATE_IDLE);
          break;
        }
        ...
        if (_use_subt_planner_) {
          ...
          // | -------------------- PAIRS SubT planner -------------------- |
          pairs_subt_planning::AstarPlanner subt_planner = pairs_subt_planning::AstarPlanner();
          ...
          ROS_INFO("[PairsOctomapPlanner]: Calling find path method.");
      ...
      if (waypoints.second) {
        ...
        ROS_INFO("[PairsOctomapPlanner]: Path is complete. Path length = %lu", waypoints.first.size());

      } else {

        ROS_INFO("[PairsOctomapPlanner]: Path is not complete");
        if (waypoints.first.size() < 2) {

          ROS_WARN("[PairsOctomapPlanner]: path not found");
        ...
        ROS_INFO("[PairsOctomapPlanner]: Path is not complete but found");
        ...
        if (path_start_end_dist < _min_path_length_) {
          ...
          ROS_WARN("[PairsOctomapPlanner]: path too short, length: %.3f", path_start_end_dist);
        ...
        if (!detected_collision_ && !first_planning_for_current_goal_ && path_end_to_goal_dist > _goal_reached_dist_ &&
          ...
          if (dist_to_goal_prev - dist_to_goal_current < _min_dist_to_goal_improvement_) {
            ...
            ROS_INFO_THROTTLE(1.0, "[PairsOctomapPlanner]: Insufficient improvement in dist to goal. Aborting the planning to avoid oscillations.");
...

      {
        ...
        pairs_msgs::ReferenceStamped position_cmd_ref;
        ...
        if (!res) {
          ROS_ERROR("[PairsOctomapPlanner]: could not transform position cmd to the map frame");
          return;
        }
      ...
      ROS_INFO("[PairsOctomapPlanner]: Calling path service with timestamp = %.3f at time %.3f.", path_stamp.toSec(), ros::Time::now().toSec());
      ros::Time tg_start = ros::Time::now();

      pairs_msgs::GetPathSrv srv_get_path;
      ...
      for (int i = 0; i < waypoints.first.size(); i++) {
        ...
        if (i > 0) {
          ...
          if (waypoint_dist > _max_segment_length_for_heading_sampling_) {
            pairs_msgs::Reference inter_ref;
            ...
            /* ROS_INFO("[PairsOctomapPlanner]: TG inter input point %02d: [%.2f, %.2f, %.2f, %.2f]", i, inter_ref.position.x, inter_ref.position.y, */
            /* inter_ref.position.z, inter_ref.heading); */
          }
        }

        ROS_INFO("[PairsOctomapPlanner]: TG input point %02d: [%.2f, %.2f, %.2f, %.2f]", i, ref.position.x, ref.position.y, ref.position.z, ref.heading);
        ...
        if (i > 1 && cum_time > _trajectory_generation_input_length_) {
          ROS_INFO("[PairsOctomapPlanner]: cutting path in waypoint %d out of %d", i, int(waypoints.first.size()));
          break;
        }
      }

      if (interrupted_) {
        ROS_WARN("[PairsOctomapPlanner]: planner interrupted, breaking main timer");
        break;
      }

      ROS_INFO("[PairsOctomapPlanner]: calling trajectory generation");
...

      {
        ...
        if (!success) {
          ROS_ERROR("[PairsOctomapPlanner]: service call for trajectory failed");
          break;
        } else {
          if (!srv_get_path.response.success) {
            ROS_ERROR("[PairsOctomapPlanner]: service call for trajectory failed: '%s'", srv_get_path.response.message.c_str());
            break;
          }
        }
      }

      ROS_INFO("[PairsOctomapPlanner]: Trajectory generation took %.2f s", (ros::Time::now() - tg_start).toSec());
      ...
      for (int i = 0; i < trajectory.points.size() - 1; i++) {
        ...
        if (octree->computeRayKeys(point1, point2, key_ray)) {
          for (octomap::KeyRay::iterator it1 = key_ray.begin(), end = key_ray.end(); it1 != end; ++it1) {
            auto node = octree->search(*it1);
            if (node && octree->isNodeOccupied(node)) {
              ROS_ERROR_THROTTLE(0.1, "[PairsOctomapPlanner]: trajectory check found collision with prediction horizon between %d and %d, replanning!", i, i + 1);
              ray_is_cool = false;
              break;
            }
          }
        } else {
          ROS_ERROR_THROTTLE(0.1, "[PairsOctomapPlanner]: trajectory check failed, could not raytrace!");
          ray_is_cool = false;
          break;
        }
      }
      ...
      ROS_INFO("[PairsOctomapPlanner]: publishing trajectory reference");

      pairs_msgs::TrajectoryReferenceSrv srv_trajectory_reference;
      ...
      ROS_INFO("[PairsOctomapPlanner]: Calling trajectory service with timestamp = %.3f at time %.3f.",
...

      {
        ...
        if (!success) {
          ROS_ERROR("[PairsOctomapPlanner]: service call for trajectory reference failed");
          break;
        } else {
          if (!srv_trajectory_reference.response.success) {
            ROS_ERROR("[PairsOctomapPlanner]: service call for trajectory reference failed: '%s'", srv_trajectory_reference.response.message.c_str());
            break;
          }
        }
      }
    ...
    case STATE_MOVING: {
      ...
      ROS_INFO_THROTTLE(1.0, "[PairsOctomapPlanner]: dist to goal: %.2f m", dist_to_goal);

      if (dist_to_goal < _goal_reached_dist_) {
        ROS_INFO("[PairsOctomapPlanner]: user goal reached");
        changeState(STATE_IDLE);
        break;
      }
      ...
      if ((ros::Time::now() - (time_last_plan_ + ros::Duration(_replan_after_))).toSec() > 0) {
        ...
        ROS_INFO("[PairsOctomapPlanner]: triggering replanning");
...
void OctomapPlanner::timerFutureCheck([[maybe_unused]] const ros::TimerEvent& evt) {
  ...
  ROS_INFO_ONCE("[PairsOctomapPlanner]: future check timer spinning");

  const pairs_lib::ScopeTimer timer = pairs_lib::ScopeTimer("timerFutureCheck", ros::Duration(_scope_timer_duration_), _scope_timer_enabled_);
  ...
  if (!octree->getRoot()) {
    ROS_ERROR("[PairsOctomapPlanner]: cannot check for collision, don't have a map");
    return;
  }

  pairs_msgs::MpcPredictionFullState            prediction           = sh_tracker_cmd_.getMsg()->full_state_prediction;
  ...
  if (control_manager_diag->flying_normally && control_manager_diag->tracker_status.have_goal) {
    ...
    if (!ret) {
      ROS_ERROR_THROTTLE(1.0, "[PairsOctomapPlanner]: could not transform position cmd to the map frame! can not check for potential collisions!");
      return;
    }
    ...
    pairs_msgs::TrajectoryReference trajectory;
    ...
    for (int i = 1; i < prediction.position.size(); i++) {
      ...
      pairs_msgs::ReferenceStamped pose;
      ...
      for (double phi = -M_PI; phi < M_PI; phi += angle_step) {
        ...
        if (octree->computeRayKeys(point1, point_ray_end, ray)) {
          for (octomap::KeyRay::iterator it = ray.begin(), end = ray.end(); it != end; ++it) {
            ...
            if (!node) {
              /* ROS_WARN("[PairsOctomapPlanner]: Detected UNKNOWN space along the planned trajectory!"); */
            } else if (octree->isNodeOccupied(node)) {
              /* ROS_WARN("[PairsOctomapPlanner]: Detected OCCUPIED space along the planned trajectory!"); */
              ...
              ROS_WARN("[PairsOctomapPlanner]: Detected OCCUPIED space along the planned trajectory! Cropped the trajectory to %d from %d points.",
          ...
          ROS_WARN_THROTTLE(1.0, "[PairsOctomapPlanner]: Unable to raycast.");
        }
        if (cropped_trajectory) {
          break;
        }
    ...
    for (int i = 0; i < trajectory.points.size() - 1; i++) {
      ...
      if (octree->computeRayKeys(point1, point2, key_ray)) {
        ...
        if (!ray_is_cool) {
          ...
          ROS_ERROR_THROTTLE(0.1, "[PairsOctomapPlanner]: future check found collision with prediction horizon between %d and %d, hovering!", i, i + 1);
        ...
        ROS_ERROR_THROTTLE(0.1, "[PairsOctomapPlanner]: future check failed, could not raytrace!");
        hover();
        break;
      }
    }
...
void OctomapPlanner::timerDiagnostics([[maybe_unused]] const ros::TimerEvent& evt) {
  ...
  if (_restart_planner_on_deadlock_ && planner_time_flag != ros::Time(0)) {
    if ((ros::Time::now() - planner_time_flag).toSec() > planner_deadlock_timeout_) {
      ROS_ERROR("[PairsOctomapPlanner]: Planner is deadlocked, restarting!");
      ros::shutdown();
    }
  }
}
...
void OctomapPlanner::setReplanningPoint(const pairs_msgs::TrajectoryReference& traj) {
...
void OctomapPlanner::changeState(const State_t new_state) {
  ...
  if (interrupted_ && old_state == STATE_IDLE) {
    ROS_WARN("[PairsOctomapPlanner]: Planning interrupted, not changing state.");
    return;
  }
  ...
  ROS_INFO("[PairsOctomapPlanner]: changing state '%s' -> '%s'", _state_names_[old_state].c_str(), _state_names_[new_state].c_str());
...
std::optional<pairs_msgs::ReferenceStamped> OctomapPlanner::getInitialCondition(const ros::Time des_time) {

  const pairs_msgs::MpcPredictionFullState prediction_full_state = sh_tracker_cmd_.getMsg()->full_state_prediction;

  if (prediction_full_state.input_id != 0 && prediction_full_state.input_id < path_id_) {
    ROS_ERROR_THROTTLE(1.0, "[PairsOctomapPlanner]: could not obtain initial condition, the input_id (%lu) does not match id of last sent path (%d).",
                       prediction_full_state.input_id, path_id_);
    return {};
  }

  if (prediction_full_state.stamps.size() == 0) {
    ROS_ERROR_THROTTLE(1.0, "[PairsOctomapPlanner]: Could not obtain initial condition, prediction full state is empty.");
    return {};
  }

  if ((des_time - prediction_full_state.stamps.back()).toSec() > 0) {
    ROS_ERROR_THROTTLE(1.0, "[PairsOctomapPlanner]: could not obtain initial condition, the desired time is too far in the future");
    return {};
  }

  pairs_msgs::ReferenceStamped orig_reference;
  ...
  if (result) {
    ...
    pairs_msgs::ReferenceStamped transformed_reference = result.value();
...
void OctomapPlanner::hover(void) {
  ...
  ROS_INFO("[PairsOctomapPlanner]: triggering hover, interrupting planner");
...
std::vector<double> OctomapPlanner::estimateSegmentTimes(const std::vector<Eigen::Vector4d>& vertices, const bool use_heading) {
  ...
  const pairs_msgs::DynamicsConstraintsConstPtr constraints = sh_constraints_.getMsg();
...
std::optional<OcTreePtr_t> OctomapPlanner::msgToMap(const octomap_msgs::OctomapConstPtr octomap) {
  ...
  if (!abstract_tree) {
    ...
    ROS_WARN("[PairsOctomapPlanner]: octomap message is empty!");
    return {};
...
void OctomapPlanner::addVirtualObstaclesToOctree(const std::shared_ptr<OcTree_t> octree) {
  ...
  for (const auto& obst : virtual_obstacles_) {
    ...
    if (obst.frame_id != octree_frame_) {
      ROS_WARN_THROTTLE(1.0, "[PairsOctomapPlanner] not adding virtual obstacle because it's frame_id (%s) doesn't match octree_frame (%s)",
...
PLUGINLIB_EXPORT_CLASS(pairs_octomap_planner::OctomapPlanner, nodelet::Nodelet)
